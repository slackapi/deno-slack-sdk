...
* invoke it with a slash command via the [Shortcut menu](/interactivity/shortcuts#global)
...
### Create a link trigger with interactivity
...
### Create a link trigger with additional parameters
...
When you need to [modify, remove, or otherwise maintain the triggers you've created](/automation/triggers/manage), the trigger ID will come in handy to specify which trigger you're updating. No need to memorize it though, since you can use `slack trigger list` to show all available triggers for your app.
...
The `inputs` of a trigger map to the inputs of a [workflow](/automation/workflows). You can pass any value as an input. You should either provide a `value` for every input, or mark the input as `customizable: true` instead. See [workflow buttons](#workflow-button-trigger) for info on why you might want to use `customizable: true`.
...
After you have created a link trigger with customizable input parameters, you can use it in a [`workflow_button`](/reference/block-kit/block-elements/#workflow_button), within which you will actually provide the values for the customizable input parameters.

Workflow buttons are only supported in messages and message attachments, and not on views.

You can use [`workflow_buttons`](/reference/block-kit/block-elements/#workflow_button) with any Slack function that accepts `interactive_blocks`, like [`SendMessage`](/reference/functions/send_message) and [`SendDm`](/reference/functions/send_dm).
...
You can also use [`workflow_buttons`](/reference/block-kit/block-elements/#workflow_button) when you call [`chat.postMessage`](/methods/chat.postMessage), [`chat.scheduleMessage`](/methods/chat.scheduleMessage), or [`chat.postEphemeral`](/methods/chat.postEphemeral) inside the `blocks` directly.
