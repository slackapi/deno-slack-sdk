...
When building workflow apps, you can use a handful of Slack types (or even [define your own](/automation/types/custom)). Slack types are used in two ways: as input and output parameters of [functions](/automation/functions/overview), and as attributes of [datastores](/automation/datastores).

## Common properties {#common-props}
...
When defining a [datastore](/automation/datastores), you can use certain Slack types for its attributes. Attributes accept only a single `type` property in the definition, instead of all the common properties listed above. The following is a list of the Slack types supported for use with datastores:

* [`channel_id`](/automation/types#channelid)
* [`date`](/automation/types#date)
* [`message_ts`](/automation/types#message-ts)
* [`rich_text`](/automation/types#rich-text)
* [`timestamp`](/automation/types#timestamp)
* [`user_id`](/automation/types#userid)
* [`usergroup_id`](/automation/types#usergroupid)
...
## Slack types catalog {#all}
| Name | Type | Description |
| ---- | --------- | ------------------ |
| [`array`](/automation/types#array) | Array | An array of items (based on a [type that you specify](/automation/types#array)).
| [`blocks`](/automation/types#blocks) | Array of Slack Blocks | An array of objects that contain layout and style information about your message. |
| [`boolean`](/automation/types#boolean) | Boolean | A logical value, must be either `true` or `false`. |
| [`canvas_id`](/automation/types#canvasid) | String | A Slack canvas ID, such as `F123456AB`. |
| [`canvas_template_id`](/automation/types#canvastemplateid) | String | A Slack canvas template ID, such as `T5678ABC`. |
| [`channel_id`](/automation/types#channelid) | String | A Slack channel ID, such as `C123ABC456` or `D123ABC456`. |
| [`date`](/automation/types#date) | String | A string containing a date, format is displayed as `YYYY-MM-DD`. |
| [`expanded_rich_text`](/automation/types#expandedrichtext) | Object | A way to nicely format messages in your app. This type cannot convert other message types, e.g. blocks or strings, and is explicitly for use with canvases. |
| [`file_id`](/automation/types#fileid) | Object | A file ID, such as `F123ABC456`. |
| [`integer`](/automation/types#integer) | Integer | A whole number, such as `-1`, `0`, or `31415926535`. |
| [`interactivity`](/automation/types#interactivity) | Object | An object that contains context about the interactive event that led to opening of the form. |
| [`list_id`](/automation/types#list-id) | String | A Slack list ID, such as `F123456ABC`. | 
| [`message_context`](/automation/types#message-context) | Object | An individual instance of a message. |
| [`message_ts`](/automation/types#message-ts) | String | A Slack-specific hash/timestamp necessary for referencing events like messages in Slack. |
| [`number`](/automation/types#number) | Number | A number that allows decimal points such as `13557523.0005`. |
| [`oauth2`](/automation/types/#oauth2) | Object | The OAuth2 context created after authenticating with [external auth](/automation/external-auth). |
| [`object`](/automation/types#object) | Object | A custom Javascript object, like `{"site": "slack.com"}`. |
| [`rich_text`](/automation/types#rich-text) | Object | A way to nicely format messages in your app. This type cannot convert other message types e.g. blocks, strings. |
| [`string`](/automation/types#string) | String | UTF-8 encoded string, up to 4000 bytes. |
| [`team_id`](/automation/types#team_id) | String | A Slack team ID, such as `T1234567890`. |
| [`timestamp`](/automation/types#timestamp) | Integer | A Unix timestamp in seconds. Not compatible with Slack message timestamps - use [string](#string) instead. |
| [`user_context`](/automation/types#usercontext) | Object | Represents a user who interacted with a workflow at runtime. |
| [`user_id`](/automation/types#userid) | String | A Slack user ID, such as `U123ABC456` or `W123ABC456`. |
| [`usergroup_id`](/automation/types#usergroupid) | String | A Slack usergroup ID, such as `S123ABC456`. |
...
In this example function, we have an array of the [custom type](/automation/types/custom) `ChannelType` as both an `input_parameter` and `output_parameter`. See this custom type and array in action in the [Deno Archive Channel](https://github.com/slack-samples/deno-archive-channel) sample app.
...
Declare an array of [Block Kit](/block-kit) JSON objects.
...
If you use [Block Kit builder](/tools/block-kit-builder) to build your Block Kit objects, be sure to _only_ grab the `blocks` array. For example:
...
| Property | Type | Description |
| ---- | --------- | ------------------ |
| `choices` | EnumChoice[] | Defines labels that correspond to the `enum` values. For more details, refer to [choices](#choices). |
...
| Property | Type | Description |
| -------- | --------- | ----------- |
| `rich_text_header` | object | The text for the header, in the form of a `plain_text` [text object](/reference/block-kit/composition-objects#text). |
| `rich_text_divider` | object | Creates a divider to place between text. |
| `rich_text_list` | object | This is an expanded version of the [`rich_text_list`](/reference/block-kit/blocks#rich_text_list) element used in `rich_text` blocks. It behaves the same, except that it accepts two new style fields: `checked` and `unchecked`. This allows for the creation of checklists. |
...
In addition to the properties [noted above](#common-props), a `string` type also has these optional properties available:
...
| `allowed_filetypes` | string[] | If provided, only these filetypes are allowed for file upload in an `OpenForm` function. Empty arrays are not allowed. A list of filetypes supported by Slack can be found [here](/types/file#types). Filetypes defined here will override any restrictions set in `allowed_filetypes_group`. |
...
In addition to the properties [noted above](#common-props), an `integer` type also has these optional properties available:
...
In addition to the properties [noted above](#common-props), the `interactivity` type also has these properties available:
...
The `message_context` type is used in the [`ReplyInThread`](/reference/functions/reply_in_thread) Slack function as the target message you want to reply to.

For example, let's say you have a workflow step that uses the [`SendMessage`](/reference/functions/send_message) function. If you want to send a reply to that message in a follow-on
step that calls the [`ReplyInThread`](/reference/functions/reply_in_thread) function, pass
...
|----------|------|-------------|----------|
| `message_ts` | [Schema.slack.types.message_ts](/automation/types#message-ts) | A Slack-specific hash/timestamp necessary for referencing events like messages in Slack. | Required |
| `channel_id` | [Schema.slack.types.channel_id](/automation/types#channelid) | The ID of the channel where the message is posted. | Optional |
...
In addition to the properties [noted above](#common-props), a `number` type also has these optional properties available:
...
In this example, we use the `oauth2` type for an input parameter in a [custom function](/automation/functions/custom). To read more about a full implementation of `oauth2`, check out [External authentication](/automation/external-auth).
...
Refer to [custom types](/automation/types/custom) for more information about typed objects, including properties and how to use  [`DefineProperty`](/automation/types/custom#define-property) to enforce required properties.
...
This type is not supported for use in the [OpenForm](/reference/functions/open_form) Slack function.
...
In addition to the properties [noted above](#common-props), the `user_context` type also has these properties available:
