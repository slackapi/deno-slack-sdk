...
<ApiMethodPage jsonFile="send_message" />

---

## Usage

Sends a message to a specific channel. The `message` input only supports non-interactive [`rich_text`](/automation/types#rich-text).
...
The `send_message` function does not allow for direct messages to users &mdash; use the [`send_dm`](/reference/functions/send_dm) function instead. 

:::

### Adding interactivity with buttons {#buttons}

The `interactive_blocks` input only supports the [`button`](/reference/block-kit/block-elements#button) and [`workflow_button`](/reference/block-kit/block-elements#workflow_button) [interactive blocks](/reference/block-kit/block-elements).
