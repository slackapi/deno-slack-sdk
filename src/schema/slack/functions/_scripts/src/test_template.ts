...
const renderFunctionManifestTest = (fn: FunctionRecord) => {
  const functionName = getFunctionName(fn.callback_id);
  const defineFunctionInput = getDefineFunctionInput(fn);
  const typescript: string[] = [];
  typescript.push(
    `assertEquals(${functionName}.definition.callback_id, "${defineFunctionInput.callback_id}");`,
  );
  typescript.push(
...
const renderWorkflowStepTest = (fn: FunctionRecord) => {
  const functionName = getFunctionName(fn.callback_id);
  const defineFunctionInput = getDefineFunctionInput(fn);
  const inputParameters = requiredParametersToTypeScript(
    defineFunctionInput.input_parameters,
  );
  const typescript: string[] = [];
  typescript.push(
  ...
  typescript.push("");
  typescript.push(
    `assertEquals(actual.function_id, "${defineFunctionInput.callback_id}");`,
  );
  typescript.push(`assertEquals(actual.inputs, ${inputParameters});`);
  return `() => {${typescript.join("\n")}}`;
};

export const SlackTestFunctionTemplate = (fn: FunctionRecord) => {
  const typescript: string[] = [];
  ...
  typescript.push(renderFunctionImport(fn.callback_id));
  typescript.push("");
  typescript.push(
    `Deno.test("${
      getFunctionName(fn.callback_id)
    } generates valid FunctionManifest", ${renderFunctionManifestTest(fn)});`,
  );
  typescript.push("");
  typescript.push(
    `Deno.test("${
